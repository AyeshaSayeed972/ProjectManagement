...
interface QAFieldsFormFields {
  remarks: string
}

interface QAFieldsFormProps {
  onSubmit: (data: QAFieldsFormFields) => Promise<void>
  defaultValues?: Partial<TaskResponse>
  isLoading?: boolean
  onCancel: () => void
}

export const QAFieldsForm: React.FC<QAFieldsFormProps> = ({
  onSubmit,
  defaultValues,
  isLoading = false,
  onCancel,
}) => {
  const { register, handleSubmit } = useForm<QAFieldsFormFields>({
    defaultValues: {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="flex flex-col gap-1">
        ...
        <textarea
          id="remarks"
          rows={3}
          placeholder="QA notes or remarks..."
