...
public class JiraService : IJiraService
{
    ...
    private async System.Threading.Tasks.Task<(HttpClient client, string baseUrl)> CreateClientAsync()
    {
    ...
    private static async System.Threading.Tasks.Task EnsureSuccessAsync(HttpResponseMessage response, string context)
    {
    ...
    public async System.Threading.Tasks.Task<string> TestConnectionAsync()
    {
    ...
    public async System.Threading.Tasks.Task<JiraIssueDto> GetIssueAsync(string issueKey)
    {
        var (client, baseUrl) = await CreateClientAsync();
    ...
    public async System.Threading.Tasks.Task<JiraSearchResultDto> SearchIssuesAsync(
        string jql, string? nextPageToken = null, int maxResults = 50)
    {
        var (client, baseUrl) = await CreateClientAsync();
        ...
        {
            Issues        = issues,
            NextPageToken = root.TryGetProperty("nextPageToken", out var tokenEl) ? tokenEl.GetString() : null,
            IsLast        = root.TryGetProperty("isLast", out var isLastEl) && isLastEl.GetBoolean()
        };
    }

    public async System.Threading.Tasks.Task<string> CreateIssueAsync(
